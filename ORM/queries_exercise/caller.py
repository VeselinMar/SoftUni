...
from main_app.choices import LaptopOSChoices


def show_highest_rated_art() -> str:
    art = ArtworkGallery.objects.order_by('-rating').first()
...
def delete_negative_rated_arts():
    negative_arts = ArtworkGallery.objects.filter(rating__lt=0).delete()
...
def update_operation_systems():
    ...
    Laptop.objects.filter(brand='Apple').update(operation_system=LaptopOSChoices.MACOS)
...
def show_workouts():
    card_cross = Workout.objects.filter(workout_type__in=("Calisthenics", "CrossFit")).order_by('id')

    result = []
    for workout in card_cross:
