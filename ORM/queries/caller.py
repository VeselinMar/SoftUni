...
def find_books_by_genre_and_language(genre, language):
    return Book.objects.filter(language=language, genre=genre)
...
def delete_review_by_id(review_id):
    ...
    result = f"Review by {review.reviewer_name} was deleted"
    review.delete()
    return result
...
def change_reviewer_name(reviewer_name, new_name):
    reviews = Review.objects.filter(reviewer_name=reviewer_name)
